...
Comprehensive mapping of all Fluent UI React v9 components to the token categories and groups defined in [Token Guidance](tokens-structure://docs/token-guidance.md). This mapping establishes the systematic relationship between component types and their corresponding token organization patterns.
...
_Smallest self-contained shapes used for single, non-interactive visual elements_
...
_Compact tap/click targets that can contain atomic elements_
...
_Medium-size components that align with form fields and contain smaller elements_
...
_Larger multi-element surfaces that combine smaller components_
...
_Supporting components that provide structure and utility functions_
